...
class BaseLLMModel:
    def __init__(
        ...
        self.token_upper_limit = MODEL_TOKEN_LIMIT.get(
    ...
    def prepare_inputs(
        ...
        if files:
            ...
            reference_results = [
                [d.page_content.strip("�"), os.path.basename(
                    d.metadata["source"])]
            ...
            if websearch_engine == "google":
                search_results = search_with_google(
                    fake_inputs, google_search_api_key, google_search_cx)
            elif websearch_engine == "bing":
                search_results = search_with_bing(
                    fake_inputs, bing_search_api_key)
            elif websearch_engine == "searchapi":
                search_results = search_with_searchapi(
                    fake_inputs, searchapi_api_key)
            elif websearch_engine == "serper":
                search_results = search_with_serper(
                    fake_inputs, serper_search_api_key)
            else:
    ...
    def load_chat_history(self, new_history_file_path=None):
        ...
        try:
            ...
            if len(saved_json["chatbot"]) < len(saved_json["history"]) // 2:
                ...
                saved_json["history"] = saved_json["history"][-len(
                    saved_json["chatbot"]):]
            ...
            self.stop_sequence = list(saved_json.get(
            ...
            self.user_identifier = saved_json.get(
                "user_identifier", self.user_name)
