...
function initialize() {
    ...
    chatbot = gradioApp().querySelector('#tecdo-chatbot');
    chatbotWrap = gradioApp().querySelector('#tecdo-chatbot > .wrapper > .wrap');
    ...
    updatetecdoBtn = gradioApp().getElementById("update-tecdo-btn");
    statusDisplay = gradioApp().querySelector('#status-display');

    historySelector = gradioApp().querySelector('#history-select-dropdown');
    tecdoPopup = gradioApp().querySelector('#tecdo-popup');
    settingBox = gradioApp().querySelector('#tecdo-setting');
    // trainingBox = gradioApp().querySelector('#tecdo-training');
    popupWrapper = gradioApp().querySelector('#popup-wrapper');
    tecdoHeader = gradioApp().querySelector('#tecdo-header');
...
function setPopupBoxPosition() {
    ...
    // tecdoPopup.style.left = `${(screenWidth - popupBoxWidth) / 2}px`;
    // tecdoPopup.style.top = `${(screenHeight - popupBoxHeight) / 2}px`;
}
...
function chatbotContentChanged(attempt = 1, force = false) {
    for (var i = 0; i < attempt; i++) {
        setTimeout(() => {
            ...
            gradioApp().querySelectorAll('#tecdo-chatbot .message-wrap .message.bot').forEach(addtecdoButton);
            ...
            if (!chatbotIndicator.classList.contains('translucent')) { // message deleted
                var checkLatestAdded = setInterval(() => {
                    var latestMessageNow = gradioApp().querySelector('#tecdo-chatbot > .wrapper > .wrap > .message-wrap .message.bot.latest');
...
Enjoy our project!\n
`
console.log(`%c${makeML(tecdoInfo)}`, styleTitle1);
